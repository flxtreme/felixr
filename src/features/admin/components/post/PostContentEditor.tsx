...
export const PostContentEditor = ({
    ...
    (prefix: string) => {
      ...
      requestAnimationFrame(() => {
        ...
        ta.selectionStart = ta.selectionEnd =
          lineStart + prefix.length + line.length;
      });
    },
    [onContentChange]
  );
  ...
  const insertAt = useCallback(
    (text: string) => {
      ...
      const start = ta.selectionStart;
      const next =
        ta.value.substring(0, start) + text + ta.value.substring(start);
      onContentChange(next);
      requestAnimationFrame(() => {
        ta.focus();
    ...
    return text.split("\n").map((line, i) => {
      if (line.trim().startsWith("```")) {
        ...
        return (
          <div
            key={i}
            className="text-emerald-500 font-mono bg-emerald-500/5 whitespace-pre"
          >
            {line || " "}
          </div>
        );
          ...
          {parts.map((part, j) => {
            if (part.startsWith("`")) {
              return (
                <span
                  key={j}
                  className="text-emerald-500 font-mono bg-emerald-500/10 px-1 rounded"
                >
                  {part}
                </span>
              );
    ...
    <section className="flex flex-col flex-1 h-full">
      ...
      <div className="flex flex-col flex-1 border-t border-border overflow-hidden">
        ...
        >
          ...
          <TbBtn
            title="Underline"
            onClick={() => wrap("<u>", "</u>", "underlined text")}
          >
          ...
          <TbBtn
            title="Image"
            onClick={() => wrap("![", "](image-url)", "alt text")}
          >
